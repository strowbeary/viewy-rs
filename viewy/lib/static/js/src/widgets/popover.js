...
document.addEventListener("click", (e) => {
    document.querySelectorAll(".popover").forEach((popover) => {
        let popover_opener = document.querySelector(`[data-v-target-popover="${popover.id}"]`)
        if (
            (e.target !== popover && !popover.contains(e.target))
            && (e.target !== popover_opener && !popover_opener.contains(e.target))) {
            popover.remove();
        }
    })
});
...
export const actions = {
    ...
    async open_popover(popover_opener) {
        const existing_popover = document.getElementById(popover_opener.dataset.vTargetPopover);
        if (existing_popover) {
