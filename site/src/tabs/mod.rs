...
pub fn tabs() -> Page<'static> {
    Page::with_title("Test").with_content(
        VStack::new(Alignment::Stretch).append_child(
            TabContainer::new()
                .as_card(CardStyle::OutlinedRaised)
                .add_tab(Tab::new("Overview", uri!(tab1())))
                .add_tab(Tab::new("Projects", uri!(tab2())))
                .add_tab(Tab::new("Account", uri!(tab3()))),
        ),
    )
}
...
pub async fn tab1() -> Page<'static> {
    Page::with_title("Tab 1").with_content({
...
pub async fn tab2() -> Page<'static> {
    Page::with_title("Tab 2").with_content({
        let mut main_stack = VStack::new(Alignment::Stretch);
        main_stack
            .gap(vec![scale(4)])
            .padding(vec![scale(5)])
...
pub async fn tab3() -> Page<'static> {
    Page::with_title("Tab 3").with_content({
        let mut main_stack = VStack::new(Alignment::Stretch);
        main_stack
            .gap(vec![scale(4)])
            .padding(vec![scale(5)])
