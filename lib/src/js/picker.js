function viewySelect(detail, picker) {
  ...
  function setValue(newValue) {
    ...
    input.setAttribute(
      "aria-label",
      `${newValue.parentNode.textContent}, listbox ${pos} of ${options.length}`,
    );
    currentValue = newValue;
    field.value = newValue.parentNode.querySelector(
      "input[type='radio']",
    ).value;
  ...
  function keyboardController(e) {
    ...
    switch (keycode) {
      ...
      case 35: // END
        ...
        setChecked(
          options[options.length - 1].querySelector('input[type="radio"]'),
        );
      ...
      case 38: // UP
        ...
        dropdown.setAttribute("data-show", "data-show");
        setChecked(
          options[current > 0 ? current - 1 : 0].querySelector(
            'input[type="radio"]',
          ),
        );
        break;
      case 40: // DOWN
        ...
        setChecked(
          options[
            current < options.length - 1 ? current + 1 : options.length - 1
  ...
  setValue(
    dropdown.querySelector(
      ".picker--dropdown__dropdown__option-list__option input[checked]",
    ),
  );
...
window.addEventListener("startViewy", ({ detail }) => {
  ...
  detail.root.querySelectorAll(".picker--segmented").forEach((picker) => {
    console.log("picker scripted", picker);
